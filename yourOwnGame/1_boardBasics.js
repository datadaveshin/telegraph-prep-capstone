// Welcome to the final project for Telegraph Prep! Here we'll be pulling
// together everything we've learned so far in the course.

// NOTE: it is an explicit goal of this project to get you familiar with
// functional programming. Functional programming is a whole new way of thinking
// that requires some practice and experience to feel natural. These prompts are
// designed to give you experience using functional programming, so go through
// them as they're written, even if you can think of other ways of doing them
// without using functional programming.

// We've created a sparse handful of helper functions in helperFunctions.js to
// handle making things appear on the screen. If you're interested in
// understanding it, feel free to look at that file!  In programming, it's a
// super useful skill to get used to just looking at the interfaces of things,
// and not worrying too much about how they work inside. If you try to figure
// out how everything works inside you'll find yourself descending through many
// deep, dark, and scary caves that oftentimes don't expand your programming
// knowledge very much. As much as possible, try to look at only the interfaces
// (what arguments a function takes, and what it returns). I'd strongly
// recommend that you take this approach with makeGameBoard and renderGameBoard.

// Let's start off by getting used to the gameBoard and how this project is
// structured.
...
//##################################################
  ...
  // Notice that we wrap all of our code in an IIFE (immediately invoked
  // function expression). This is a common pattern that you'll see... can you
  // guess why? If you need some help, try googling "avoid polluting global
  // scope".  Since all of our code will be written inside the local scope of a
  // function, you won't be able to just type a variable name directly in the
  // console and see its value (remember, the console only has access to global
  // scope variables). So, in order to see the values of the variables you
  // declare you'll need to make use of console.log to print them to the
  // console. Writing all your console.logs in your .js file here is a good
  // pattern to get used to.
  
  // The only thing we want on our global scope is our gameBoard object. We need
  // to make gameBoard a global variable so that all the JavaScript files in our
...
//##################################################
  ...
  // we can just invoke it even though we haven't seen it declared in this
  // file... this function was defined in the helperFunctions.js file and it was
  // declared in the global scope, so it's accessible anywhere in our JavaScript
  // code.
...
console.log("##################################################")
  ...
  // gameBoard is an array of arrays (an array that is filled with other
  // arrays). Let's start by using each to iterate through the gameBoard array,
  // console.logging each item inside the gameBoard array. You should see 8
...
_.each(gameBoard[0], function (element, index, list) {
  ...
  // have many different pieces of information we want to store about each
  // square: what color it is, what position it is on the board, what gamePiece
  // is at that position, etc. An object is a perfect way to store information
  // about all these different properties associated with that square.

  // gameBoard[row][column] will get you the squareObj at that position in the
  // gameBoard. So gameBoard[2][6] will point to the squareObj on row 3 column
  // 7. Quick review on how this works: JS chains operators together, the
  // results of each one being passed to the next operation. So first we're
  // accessing the thing at index 2 in our gameBoard when we say gameBoard[2].
  // Then, within that thing (which is an array representing a row), we're
  // asking for the item at the 6th index (which is going to be a squareObj).
  // We could chain this together even more. Explain with your pair each
  // individual operation that's going on when say:   
  ...
  //#############################################
    ...
    // Now let's do the same thing using map on the second row of our gameBoard.
      ...
      // of values to you. So what we're going to do in the end is overwrite the
      // second row of the gameBoard with the mutated array map returns to us.
      // Let's pseudocode!
        // map through an array of colors remember that the callback function
        // has to do three three things:
          // 1. Take in an item. 2. Do something to change that item into what
          // we want to see in the output array. 3. Return the changed item.
        // On each iteration of map
          // Take in a color Change that color into a squareObj
            // Recall that each squareObj on our board looks like this:
              // var square = {     position: [row, column],     color:
              // colorToBeDisplayed,     gamePiece: '', // This is the property
              // that will contain our gamePiece object if one is on that
              // square.      text: '' };
          // Return the squareObj Set the second row of our gameBoard equal to
          // what map returns to us.  Remember, the key difference between map
          // and each is that each purely has side effects, while map is
          // designed to return a new array that is the same length as the old
...
console.log("##################################################")
  ...
  //#############################################
      ...
      // for loops, let's transition this over to functional programming.
        ...
        // the color of each square to blue. Be sure to write a new each
        // statement for this- don't just copy and paste the one you've written
...
console.log("##################################################")
  ...
  //#############################################
          ...
          // at each step!
            ...
            // working with; you'll have to log the results to see what the
            // variables represent.
          // This is where naming your variables something descriptive makes a
...
//## passed. I renamed these already to be reflected in the code.
      ...
      // that you can make that code do whatever you want it to.
        // Let's replace our inner each loop with map, changing the colors of
...
console.log("##################################################")
  ...
  // puzzle.  As you work through this project, you will get stuck on things.
  // That's totally normal and expected! Getting your code to work when it isn't
  // is called debugging. When you get stuck, follow the debugging pattern we
  // outline below.
    // Debugging pattern:
      // 1. Verify all of your assumptions by logging them to the console with
      // clear comments on what each log is. For example: console.log('squareObj
      // inside _.each callback', squareObj); 2. Find the last point in your
      // code where things are doing what you expect them to/ the first point in
      // your code where they're not doing what you expect. 3. Iterate rapidly.
      // Try new things (and console.log them!) until you get to a solution that
      // works. Pay attention to each new result along the way (even if it's not
      // what you expect it to be) to see what new information you can learn
      // from it. 4. High five someone nearby when you figure it out!

  // Now that we have a decent understanding of the gameBoard. Now let's test
  // out the makePiece function.
    // Let's add a new piece (name it anything you want. babyDino is my current
    // favorite, but I'm sure you'll have fun coming up with your own favorite
    // gamePieces!). Try invoking makePiece with the right arguments and make
    // sure it worked by opening up your browser. Uncomment the following lines
    // to make this work.  makePiece(gameBoard, [3,5], 'babyDino');
...
console.log("##################################################")
  ...
  //#############################################
    ...
    // Now that we've added a piece to the board, let's use that piece to
    // practice filter. If you're not familiar with filter, look it up in the
    // underscorejs docs!  Invoke _.filter on the row that you just added the
    // gamePiece to. See if you can use it to return an array of only the
...
//#############################################
  ...
  // and your code look more professional.

  // We're now going to go on a mini-sprint covering reduce, the last critical
  // functional programming tool. Understanding reduce will let us take this
  // messy nested array we have, and turn it into a single, flattened array.

  // CARRY ON...   // Go ahead and jump to the file called '2_reduceIntro.js' in 
  // the yourOwnGame folder. 
})();
